  ...
  <script type="text/javascript">
 	 $(document).ready(function(){
 		$("#allProForm").hover(function(){
			...
			$("#allProView").html("<table border='1'><tr><td colspan='2' width='500px'>생성중</td></tr><c:forEach items='${requestScope.pmvo.creatingProject}' var='createP'><tr><td>${createP.projectName}</td><td>${createP.project_date}</td></tr></c:forEach>"+
				"<tr><td colspan='2'>신청중</td></tr><c:forEach items='${requestScope.pmvo.joinProject}' var='joinP'><tr><td>${joinP.projectName}</td><td>${joinP.deadline}</td></tr></c:forEach><tr><td colspan='2'>진행중</td></tr><c:forEach items='${requestScope.pmvo.processingProject}' var='processP'><tr><td>${processP.projectName}</td><td>${processP.project_date}</td></c:forEach></table>");
  		},function(){
 		...
 		$("#regProForm").hover(function(){
  			...
  			$("#regProView").html("<c:choose><c:when test='${fn:length(requestScope.pmvo.creatingProject)!=0}'><br><c:forEach items='${requestScope.pmvo.creatingProject}' var='createP' varStatus='start'><table border='1'><tr><td><a href='project_View.do?projectNo=${createP.projectNo}'>${createP.projectName}</a></td></tr> "+
  			...
  			"<tr><td><c:forEach items='${createP.commentVO }' var='com'><br><table border='1'><tr><td width='120px'>ID</td><td width='200px'>한마디</td><td width='150px'>접수일</td></tr><tr><td>${com.id}</td><td>${com.content}</td><td>${com.commentDate}</td></tr></table></c:forEach><br></c:forEach></td></tr></table></c:when><c:otherwise></c:otherwise></c:choose>");
  		},function(){
 		...
 		$("#processProForm").hover(function(){
  			...
  			$("#processProView").html("<c:choose><c:when test='${fn:length(requestScope.pmvo.processingProject)!=0}'><table><c:forEach items='${requestScope.pmvo.processingProject}' var='processP'><tr><td width='290px'><a href='project_View.do?projectNo=${processP.projectNo}'>${processP.projectName}</a></td><td> ${processP.progressing}% </td><td>${processP.project_date}</td></tr><br></c:forEach></table><br><br></c:when><c:otherwise></c:otherwise></c:choose>");
  		},function(){
