...
<c:forEach items="${requestScope.exception}" var="i">
	${i.errorNo } , ${i.exceptionMessage } , ${i.id } , ${i.content } , ${i.hit } <br>
</c:forEach>


<div class="page">
	<a href="search_exceptionListView.do?pageNo=${requestScope.vo.pagingBean.startPageOfPageGroup-1}&wordName=${requestScope.wordName}">
	...
	<c:forEach var="i" begin="${requestScope.vo.pagingBean.startPageOfPageGroup}" end="${requestScope.vo.pagingBean.endPageOfPageGroup}" step="1">
		<c:choose>
			...
			<c:otherwise>
				<a href="search_exceptionListView.do?pageNo=${i}&wordName=${requestScope.wordName}">${i}</a>
			</c:otherwise>
		</c:choose>
	</c:forEach>
	
	<a href="search_exceptionListView.do?pageNo=${requestScope.vo.pagingBean.endPageOfPageGroup+1}&wordName=${requestScope.wordName}">
