...
<c:forEach items="${requestScope.discuss}" var="i">
	${i.discussionNo } , ${i.id } , ${i.title } , ${i.content } , ${i.discussionDate } , ${i.hit }, ${i.discussionSubject} <br>
</c:forEach>


<div class="page">
	<a href="search_discussListView.do?pageNo=${requestScope.vo.pagingBean.startPageOfPageGroup-1}&wordName=${requestScope.wordName}">
	...
	<c:forEach var="i" begin="${requestScope.vo.pagingBean.startPageOfPageGroup}" end="${requestScope.vo.pagingBean.endPageOfPageGroup}" step="1">
		<c:choose>
			...
			<c:otherwise>
				<a href="search_discussListView.do?pageNo=${i}&wordName=${requestScope.wordName}">${i}</a>
			</c:otherwise>
		</c:choose>
	</c:forEach>
	
	<a href="search_discussListView.do?pageNo=${requestScope.vo.pagingBean.endPageOfPageGroup+1}&wordName=${requestScope.wordName}">
